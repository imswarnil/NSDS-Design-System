...
<div class="ns-banner ns-banner--success">
  ...
  },
  {
    ...
    use: ["A training post in a curriculum built for ~100 sections and ~500 lessons", "--doc for the document layout: content full width, curriculum on the trailing edge", "--fixed for the older two-column form with the rail sticky rather than pinned"],
    ...
    a11y: [
      "The reading column has NO width cap. The thing a cap protects is line length, and .ns-prose already caps its own paragraphs at --measure-prose — so a sentence never runs past 68ch while the tables, code, video stage and pager get the whole column, which is what they were always short of. Cap the paragraph, not the page",
      "A sticky TOPBAR over the reading column carries what a reader glances up for: crumb, a meter for how far through the section, prev/next, and the way into the curriculum. Below md the meter goes and below lg the steps go — on a phone the bar is the crumb and the drawer handle, not six controls in a 360px row",
      ...
      "BUILT FOR ~100 SECTIONS AND ~500 LESSONS, and that number changes the design rather than just stressing it. Sections are CLOSED by default with only the current one open, so the rail is ~100 scannable rows instead of a 500-row document; every section is NUMBERED, because at a hundred a title is one phrase among many and the number is what makes a position; and each section summary is STICKY, so scrolling a 40-lesson section never leaves you among titles with no idea which section owns them. assets/js/training.js opens the current section and scrolls it into view on load, so the rail opens already showing where you are",
      "It is deliberately NOT virtualised. 500 anchors is a trivial DOM, and virtualising breaks the browser's own find-in-page — which is the feature learners actually use to find a lesson",
      "The rail is a THREE-part grid — pinned head, scrolling middle, pinned foot — so the filter and the actions are on screen for the whole lesson. The head is the FILTER and nothing else: a track header there restated the topbar's own crumb and meter, spending three rows of the narrowest column on the page to repeat what was already on screen. &ldquo;Find me the lesson about X&rdquo; is the one question that cannot be answered by something that scrolled away 400 rows ago",
      "The foot carries share and Ask AI, and the panel takes .ns-askai--end because the rail is on the TRAILING edge — the base panel opens from its start edge, which is right in the course player (rail on the left) and opens entirely off-screen here",
      ...
      "Below lg the rail becomes a drawer from the TRAILING edge — the edge its column lives on — closed by the ✕, the scrim and Escape. The scrim sits BEFORE the rail in the markup so the rail, also fixed at --z-overlay, paints above it without inventing a seventh elevation level",
      "The reading rail carries PROVENANCE then the ad. Author and last-updated live there rather than in the header: nobody checks who wrote a lesson before starting it and everybody checks after hitting something surprising, which makes them things you reach for WHILE reading. The header keeps only what gates the decision to start — duration, position, difficulty, read state — and the kind moved up to a mark above the title",
      "A SKYSCRAPER SLOT (160&times;600, the standard unit from ads.css) sits under the provenance, sticky beside the reading. It begins BELOW the header, which owns the full width — a column starting at the top would push the title into two thirds of a page it should own outright. Below xl the whole column becomes a scrollable row under the header, and ads.css drops the 600px unit on a phone",
